...
const CLASS_TRAINING_KEY   = ['class-training']
...
export function useRealtimeSync(userId, heroId) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`hero:${heroId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'heroes',          filter: `id=eq.${heroId}` },      () => { if (qc.isMutating({ mutationKey: TRAINING_ASSIGN_KEY }) === 0 && qc.isMutating({ mutationKey: CLASS_TRAINING_KEY }) === 0 && qc.isMutating({ mutationKey: STRATEGY_KEY }) === 0) { qc.invalidateQueries({ queryKey: queryKeys.hero(heroId) }); qc.invalidateQueries({ queryKey: queryKeys.heroes(userId) }) } })
