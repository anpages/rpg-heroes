-- Fix: al recoger, resetear production_collected_at a clock_timestamp().
-- Bug anterior: avanzaba el timestamp un ciclo, permitiendo recoger múltiples veces
-- si el almacén llevaba >1 ciclo lleno.
...
BEGIN
  ...
  -- Reset a now: el ciclo de producción arranca desde cero
  UPDATE buildings
  SET production_collected_at = clock_timestamp()
  WHERE id = v_bld.id;
