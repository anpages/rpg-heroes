-- RPC atómica para equipar/desequipar tácticas
-- Reemplaza 4-6 queries secuenciales por 1 sola llamada

CREATE OR REPLACE FUNCTION equip_tactic(
  p_hero_id    UUID,
  p_tactic_id  UUID,
  p_slot_index INT,   -- NULL para desequipar
  p_user_id    UUID,
...
DECLARE
  ...
  v_ok                BOOLEAN;
BEGIN
  -- 1. Verificar héroe pertenece al usuario y no está en expedición
  ...
  -- 2. Verificar que el héroe tiene esta táctica + restricción de clase
  ...
  -- 3. Si ya está en el slot destino, no hacer nada
  IF v_hero_tactic.slot_index IS NOT DISTINCT FROM p_slot_index THEN
    RETURN jsonb_build_object('ok', true, 'changed', false);
  END IF;

  -- 4. Cobrar gold si es un swap (equipado → equipado distinto)
  ...
  -- 5. Si el slot destino está ocupado, desequipar al ocupante
  v_occupant_id := NULL;
  ...
  -- 6. Actualizar slot de la táctica
